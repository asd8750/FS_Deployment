...
# =========================================================================
function Deploy-MirrorEndPointCredAuth  {
    <#
.SYNOPSIS

Create the infrastructure to use SQL mirroring endpoints using credential authentication
...
Version History
- 2022-04-20 - 1.0 - F.LaForest - Initial version
...
.PARAMETER Command
This function performs a number of functions.  Use if only one action is required
    VALIDATE - Connect to each of the specified instances and determine what further actions are needed
    CREATE - Create the certificates, logins/users and endpoints are needed
    SAVE - Save the information gathered and the deployment state
    ...
    begin {
        ...
        foreach ($instName in $InstanceList) {
            try {
                ...
                #
                $oInfo = (Invoke-Sqlcmd -ServerInstance $instName -Database 'master' -Query "
                ...
                #   Get the list of installed certificates used for mirroing plus user/login/permission info
                #
                $instKeys = Get-CertificateInfo -ServerInstance $instName #-Database 'master' 
                ...
                $EPInfo = Get-EndpointInfo -ServerInstance $instName # -Database 'master'
        ...
        foreach ($InstInfo in $InstanceInfo) {
            ...
            if (-not $InstInfo.MasterKey) {
                ...
                if ($DebugLevel -le 1) {Invoke-Sqlcmd -ServerInstance $InstInfo.ListedName -Database 'master' -Query "CREATE MASTER KEY ENCRYPTION BY PASSWORD = '$($MasterCertPassword)';"}
            }
            else {
...
# $InstanceList_PMT1_SqlProd    = @("PKM1SQL20T101.mfg.fs","PKM2SQL20T101.mfg.fs")
# $InstanceList_PMT1_SqlSpc     = @("PKM1SQL20T102.mfg.fs","PKM2SQL20T102.mfg.fs")
# $InstanceList_PMT1_SqlMisc    = @("PKM1SQL20T103.mfg.fs","PKM2SQL20T103.mfg.fs")
# $InstanceList_PMT1_SqlPrcData = @("PKM1SQL20T104.mfg.fs","PKM2SQL20T104.mfg.fs")
# $InstanceList_PMT1_SqlODS     = @("PKM1SQL01T104.fs.local","PKM2SQL01T104.fs.local")

# $InstanceList_PMT1_PDR_0 = @("PKM1SQL20T140.mfg.fs","PKM2SQL20T140.mfg.fs")
# $InstanceList_PMT1_PDR_1 = @("PKM1SQL20T141.mfg.fs","PKM2SQL20T141.mfg.fs")
# $InstanceList_PMT1_PDR_2 = @("PKM1SQL20T142.mfg.fs","PKM2SQL20T142.mfg.fs")
# $InstanceList_PMT1_PDR_3 = @("PKM1SQL20T143.mfg.fs","PKM2SQL20T143.mfg.fs")


if (-not $FSDeploymentIsLoading) {

    #Deploy-MirrorEndPointCredAuth -InstanceList $InstanceList_PMT1_SqlPrcData -Command VALIDATE -Verbose -DebugLevel 1
    # $nodes = $InstanceList_PMT1_SqlMisc
