...
function Get-FSSQLTableCreateScript {
    ...
    $scriptrCreate.Options.NoIdentities = ($NoIdentity -ne 0)
...
if (-not $FSDeploymentIsLoading){
    ...
    Deploy_PVTables -InstanceName "PBG1SQL01V001.fs.local" -DatabaseName "ReliabilityDB" `
                                -TableSchema "dbo" -TableName 'RCOL_TemperatureMap' `
                                -StgTableSchema "DBA-Stg" `
                                -PtSchemeName "source" -PtColumnName "DateUpdated";
}